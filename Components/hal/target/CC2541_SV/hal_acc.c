...
*/
// Sensor I2C address
#define HAL_KXTI9_I2C_ADDRESS         0x53 //0x1D // 0x0F
...
#define ACC_REG_ADDR_XOUT_L            0x32 // 0x06 // R
#define ACC_REG_ADDR_XOUT_H            0x33 // 0x07 // R
#define ACC_REG_ADDR_YOUT_L            0x34 // 0x08 // R
#define ACC_REG_ADDR_YOUT_H            0x35 // 0x09 // R
#define ACC_REG_ADDR_ZOUT_L            0x36 // 0x0A // R
#define ACC_REG_ADDR_ZOUT_H            0x37 // 0x0B // R
...
#define ACC_REG_ADDR_CTRL_REG1        0x2D// 0x1B // R/W
...
static uint16 AccBuf[30];                      // Acc Data buffer

static halACCBufControl_t halACCBuf;
...
{
  halACCBuf.bufferHead = AccBuf;
  halACCBuf.bufferTail = AccBuf + (sizeof (AccBuf) / sizeof( AccBuf[0]));
  halACCBuf.maxBufSize = sizeof (AccBuf) / sizeof( AccBuf[0]);
  halACCBuf.pBuffer = AccBuf;
...
* @return      None.
*/
void HalAccBufWrite( uint8 param, uint8 len, void *value )
{
    uint8* pNext;
       pNext = osal_memcpy( halACCBuf.pBuffer, value, len );
       halACCBuf.pBuffer = (uint16 *) pNext;

     if (halACCBuf.pBuffer >= halACCBuf.bufferTail)
     {
       osal_set_event( 0x03, ST_ACCELEROMETER_FULL_BUFFER);
       halACCBuf.pBuffer = halACCBuf.bufferHead;
     }
}
/**************************************************************************************************
* @fn          filterAccData
*
* @brief       This function filter Acc data.
*
* @return      None.
*/
void filterAccData(void)
{
  uint16 *next = halACCBuf.bufferHead;
  ...
  uint16 maxZ;
    ...
    int i;
    for(i=0;i<sizeof (AccBuf) / sizeof( AccBuf[0]); i++) // filter adjust buffer
      {
          *next += 0x400;
          next++;
      }

 next = halACCBuf.bufferHead;
    minX = maxX = *next++;
    minY = maxY = *next++;
    minZ = maxZ = *next;
      for(i=0;i<((sizeof (AccBuf) / sizeof( AccBuf[0]) / 3) - 1); i++) // filter 5 points find max
      {
          *next++;
          minX = MIN(minX,*next);
          maxX = MAX(maxX,*next);
    ...
    if ((maxX - minX >  0x20) || (maxY - minY >  0x20) || (maxZ - minZ >  0x20))
    {
      osal_set_event( 0x03, ST_MOVE_DETECTED);
    }
}
/**************************************************************************************************
* @fn          HalAccSetRange
...
void HalAccSetRange(uint8 range)
{
  ...
  case HAL_ACC_RANGE_2G:
    accSensorConfig = 0x08;//ACC_REG_CTRL_ON_2G;
    accSensorOff = ACC_REG_CTRL_OFF_2G;
    break;
  case HAL_ACC_RANGE_4G:
    accSensorConfig = 0x08;//ACC_REG_CTRL_ON_4G;
    accSensorOff = ACC_REG_CTRL_OFF_4G;
    break;
  case HAL_ACC_RANGE_8G:
    accSensorConfig = 0x08;//ACC_REG_CTRL_ON_8G;
...
bool HalAccRead(uint8 *pBuf )
{
  ...
  success = HalSensorReadReg( ACC_REG_ADDR_XOUT_L, &xL, sizeof(xL));
  if (success)
  {
     success = HalSensorReadReg( ACC_REG_ADDR_XOUT_H, &xH, sizeof(xH));
     if (success)
     {
         success = HalSensorReadReg( ACC_REG_ADDR_YOUT_L, &yL, sizeof(yL));
         if (success)
         {
             success = HalSensorReadReg( ACC_REG_ADDR_YOUT_H, &yH, sizeof(yH));
             if (success)
             {
                 success = HalSensorReadReg( ACC_REG_ADDR_ZOUT_L, &zL, sizeof(zL));
                 if (success)
                 {
                     success = HalSensorReadReg( ACC_REG_ADDR_ZOUT_H, &zH, sizeof(zH));
                 }
             }
         }
     }
