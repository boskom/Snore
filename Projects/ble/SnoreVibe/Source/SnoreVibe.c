...
void init(struct buffer *b)
{
  ...
  b->size = sizeof(b->array) / sizeof(b->array[0]);
 ...
 */
uint16 SnoreVibe_ProcessEvent( uint8 task_id, uint16 events )
{
  ...
  //    Accelerometer     //
  //////////////////////////
  if ( events & ST_MOVE_DETECTED )
  {
   MOTOR_PORT &= ~(MOTOR_BIT);  /* SET LOW*/
   return (events ^ ST_MOVE_DETECTED);
  }  
    
  if ( events & ST_ACCELEROMETER_FULL_BUFFER )
  {
    filterAccData();
    return (events ^ ST_ACCELEROMETER_FULL_BUFFER);
  }
  ...
  //    Microphone        //
  //////////////////////////
  if ( events & ST_SNORING_DETECTED )
  { 
    MOTOR_PORT |= (MOTOR_BIT);    /* SET high*/
     asm("nop"); asm("nop"); asm("nop"); asm("nop"); asm("nop"); asm("nop");
     MOTOR_PORT ^= (MOTOR_BIT);
  asm("nop"); asm("nop"); asm("nop"); asm("nop"); asm("nop");asm("nop");
  MOTOR_PORT ^= (MOTOR_BIT);
       asm("nop"); asm("nop"); asm("nop"); asm("nop"); asm("nop"); asm("nop");
       MOTOR_PORT ^= (MOTOR_BIT);
  ...
  MOTOR_PORT ^= (MOTOR_BIT);
       asm("nop"); asm("nop"); asm("nop"); asm("nop"); asm("nop"); asm("nop");
  asm("nop"); asm("nop"); asm("nop"); asm("nop"); asm("nop");asm("nop");
    return (events ^ ST_SNORING_DETECTED);
  }
...
void readAccData(void)
{
  ...
  if (HalAccRead(aData))
  {
    ...
    HalAccBufWrite( SENSOR_DATA, ACCELEROMETER_DATA_LEN, aData );
  }
}
